...
-- Demo edge-case customers. These are inserted idempotently so the same SQL can
-- also be applied to an existing dev database.
...
-- Greenfield Foods: resolved issue with enough closure evidence.
INSERT INTO issues (issue_ref, customer_id, title, description, severity, status, sla_status, owner, opened_at)
...
-- Redwood Telecom: P2 at risk, stale, and unowned. Good for "urgent but not P1" demos.
INSERT INTO issues (issue_ref, customer_id, title, description, severity, status, sla_status, owner, opened_at)
...
-- Meridian Bank: Strategic P1 that is not breached yet. Useful contrast with critical breached issues.
INSERT INTO issues (issue_ref, customer_id, title, description, severity, status, sla_status, owner, opened_at)
...
-- Edge-case updates. Use NOT EXISTS guards because these are safe to replay in
